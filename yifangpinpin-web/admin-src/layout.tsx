...
async function getNavCounts() {
  ...
  const [props, demands] = await Promise.all([
  ...
  return { propCount: props.count ?? 0, demandCount: demands.count ?? 0 };
}

export default async function AdminLayout({ children }: { children: React.ReactNode }) {
  const { propCount, demandCount } = await getNavCounts();

  return (
    <div className="yfp-admin flex min-h-screen">
      <AdminSidebar propCount={propCount} demandCount={demandCount} />
