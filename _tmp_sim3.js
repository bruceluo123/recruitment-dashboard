...
// EXISTING regexes, additions marked with /*+*/
const KW = [
...
function classify(title,body){
  ...
  return sc.length?[sc[0][0]]:['operations'];
}
let changed=0;const moves={};const newCnt={};const fb=[];
for(const j of a){
  ...
  if(old!==nw){changed++;moves[old+' => '+nw]=(moves[old+' => '+nw]||0)+1;
    fb.push(j.title.replace(/\s+/g,' ')+'  ['+old+' => '+nw+']'+(detectTitle(j.title).length?' (title)':' (body)'));}
}
console.log('records:',a.length,'changed:',changed);
console.log('\n--- NEW counts ---');
console.log(Object.entries(newCnt).sort((x,y)=>y[1]-x[1]).map(e=>e[0]+': '+e[1]).join('\n'));
console.log('\n--- all moves ---');
console.log(Object.entries(moves).sort((x,y)=>y[1]-x[1]).map(e=>e[1]+'x  '+e[0]).join('\n'));
console.log('\n--- all changed records ---');
console.log([...new Set(fb)].join('\n'));
