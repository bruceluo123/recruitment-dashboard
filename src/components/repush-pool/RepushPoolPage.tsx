...
export function RepushPoolPage() {
  ...
  const toggleLedger = async () => {
    ...
    try {
      const response = await fetch(`/api/feedback-center?owner=${owner}&days=7&ledger=1`, { cache: 'no-store' });
      const data = await response.json() as FeedbackCenterState & { ok?: boolean; error?: string };
      if (!response.ok || data.ok === false) throw new Error(data.error || '读取消息台账失败');
      setMessageLedger(Array.isArray(data.ledger) ? data.ledger : []);
    } catch (ledgerError) {
  ...
  const ledgerCounts = useMemo(() => messageLedger.reduce<Record<string, number>>((result, item) => {
            ...
            {ledgerLoading ? <div className="flex h-24 items-center justify-center text-sm text-slate-400"><Loader2 className="mr-2 h-4 w-4 animate-spin" />正在读取台账</div> : messageLedger.length === 0 ? <p className="rounded-md bg-white px-4 py-5 text-center text-sm text-slate-400">台账将在下一次反馈审计后生成</p> : (
              <div className="max-h-[520px] space-y-1.5 overflow-y-auto pr-1">
                {messageLedger.map((entry) => {
                  const meta = entry.status === 'resolved'
                    ? { label: '已落实', className: 'bg-emerald-50 text-emerald-700' }
                    : entry.status === 'manual_review'
                      ? { label: '待核对', className: 'bg-violet-50 text-violet-700' }
                      : entry.status === 'failed'
                        ? { label: '处理失败', className: 'bg-rose-50 text-rose-700' }
