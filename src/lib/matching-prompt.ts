...
const SCORING_RUBRIC = `## 分析方法（先挖掘JD核心，再比对，最后打分）
对每个岗位，先在心里完成三步，再输出分数：
1. 挖掘JD核心：剥离套话，提炼这个岗位真正的「硬性要求」（必须具备，不满足=硬伤）和「加分项」（有则更好）。重点抓核心职能与方向，而非字面用词。
2. 比对证据：在简历里找对应硬性要求的事实证据。注意语义等价——简历用词可能与JD不同（如"注册转化留存"≈"用户运营增长"、"0-1搭建"≈"从无到有"），不要因为字面不同就判为不匹配。
3. 打分：硬性要求是否满足决定分数下限；加分项决定上浮。方向跨界但核心能力可迁移的，按可迁移程度给分，不要一刀切归零。
