...
(async () => {
  ...
  const mixedTalents = JSON.parse(db.get('recruit:talents'));
  mixedTalents.push({ id: 'legacy-shared-code', candidateCode: 'XYBB00142', name: 'Different' });
  db.set('recruit:talents', JSON.stringify(mixedTalents));
  await api.main({ client, write: true, dialog: 'ojisamer' });
  recs = JSON.parse(db.get('recruit:repush'));
  ...
  assert.equal(JSON.parse(db.get(stateKey)).entries.XYBB00142.name, 'bob', 'a legacy talent sharing the code cannot overwrite its registered owner');
