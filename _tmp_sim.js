...
// Proposed keyword set (expanded). Order encodes priority.
const KW = [
  ['seo', /seo|搜索引擎优化|关键词优化/i],
  ['advertising', /广告|信息流|投放|sem|feed|千川|广告素材|广告策略/i],
  ['gaming', /游戏|unity|unreal|ue[45]|cocos|fps|mmo|演武/i],
  ['ai', /(^|[\s\-_/｜|（）()【】])ai(?=$|[\s\-_/｜|（）()【】])|ai(?=[^\x00-\x7f])|人工智能|大模型|llm|gpt|prompt|aigc|\bagent\b|comfyui|效能官|ai提效|ai效能|智能体/i],
  ...
  ['hr', /hr|人力|招聘|薪酬|员工关系|组织发展|人事|人事行政|ssc|绩效|社保|考勤|薪资核算/i],
...
// strong cats: a body mention is decisive
const STRONG = new Set(['seo','advertising','gaming','ai','algorithm','frontend','backend','devops','testing','training','design','art','video','live','legal','finance','data','hardware','hr','customer-service']);
...
function classify(title, body){
  ...
  const titleCats=[]; const bodyScore=new Map();
  for(const [cat,re] of KW){
    if(re.test(t)) titleCats.push(cat);
    const h=countHits(re,b); if(h>0) bodyScore.set(cat,h);
  }
  const result=[...titleCats];
  // add strong body-only cats (>=2 hits) not already present
  const extras=[...bodyScore.entries()]
    .filter(([c,h])=>STRONG.has(c)&&h>=2&&!result.includes(c))
    .sort((x,y)=> y[1]-x[1] || ORDER.indexOf(x[0])-ORDER.indexOf(y[0]))
    .map(x=>x[0]);
  for(const c of extras){ if(result.length>=3) break; if(!result.includes(c)) result.push(c); }
  if(result.length===0){
    // body-any fallback: highest scoring cat
    const any=[...bodyScore.entries()].sort((x,y)=>y[1]-x[1]||ORDER.indexOf(x[0])-ORDER.indexOf(y[0]));
    if(any.length) result.push(any[0][0]);
    else result.push('operations');
  }
  // keep order by ORDER priority, cap 3
  return result.sort((x,y)=>ORDER.indexOf(x)-ORDER.indexOf(y)).slice(0,3);
}
...
for(const j of a){
  ...
  const old=(j.categories||[]).slice().sort().join(',');
  const nw=nc.slice().sort().join(',');
